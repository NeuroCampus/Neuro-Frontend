...
import { Calendar as CalendarIcon, Sliders } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        else console.error(json.message || 'Failed to load branches');
      } catch (e: unknown) {
...
const useFacultiesByBranch = (selectedBranch: string | null) => {
  ...
  useEffect(() => {
    ...
    const loadFaculties = async () => {
      ...
      try {
        ...
        else console.error(json.message || 'Failed to load faculties');
      } catch (e: unknown) {
        const msg = safeErrorMessage(e);
        console.error(msg);
      } finally {
...
const useFacultyProfile = (facultyId: string | null, startDate: string, endDate: string, reloadKey: number) => {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        else console.error(json.message || 'Failed to load profile');
      } catch (e: unknown) {
        const msg = safeErrorMessage(e);
        console.error(msg);
      }
    ...
    loadProfile();
    return () => { mounted = false; };
  }, [facultyId, startDate, endDate, reloadKey]);
...
const DeanFacultyProfile = ({ facultyId: initialFacultyId, initialStartDate, initialEndDate }: DeanFacultyProfileProps) => {
  ...
  const { faculties, loading: facultiesLoading } = useFacultiesByBranch(selectedBranch);
  ...
  const { profile } = useFacultyProfile(selectedFaculty, startDate, endDate, reloadKey);
            ...
            <div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-8">
                  <StatsCard label="Weekly Hours" value={profile.total_weekly_hours ?? 0} color="blue" />
                  <StatsCard label="Present Days" value={profile.attendance_summary?.present_days ?? 0} color="green" />
                  <StatsCard label="Absent Days" value={profile.attendance_summary?.absent_days ?? 0} color="red" />
                  <StatsCard label="Attendance %" value={profile.attendance_summary?.percent_present ?? 'N/A'} color="purple" />
                  <StatsCard label="Leave Days" value={profile.attendance_summary?.leave_days ?? 0} color="yellow" />
                  <StatsCard label="Unmarked Days" value={profile.attendance_summary?.unmarked_days ?? 0} color="amber" />
                </div>
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    <AssignmentsList assignments={profile.assignments ?? []} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <ScheduledClassesTable classesList={profile.scheduled_classes ?? []} />
                </div>
              </div>
            </div>
          )}
...
function StatsCard({ label, value, color }: StatsCardProps) {
  const colorMap: Record<string, { gradient: string; border: string; label: string; value: string }> = {
    blue: { gradient: 'from-blue-50 to-blue-100', border: 'border-blue-200', label: 'text-blue-600', value: 'text-blue-900' },
    green: { gradient: 'from-green-50 to-green-100', border: 'border-green-200', label: 'text-green-600', value: 'text-green-900' },
    red: { gradient: 'from-red-50 to-red-100', border: 'border-red-200', label: 'text-red-600', value: 'text-red-900' },
    purple: { gradient: 'from-purple-50 to-purple-100', border: 'border-purple-200', label: 'text-purple-600', value: 'text-purple-900' },
    yellow: { gradient: 'from-yellow-50 to-yellow-100', border: 'border-yellow-200', label: 'text-yellow-600', value: 'text-yellow-900' },
    amber: { gradient: 'from-amber-50 to-amber-100', border: 'border-amber-200', label: 'text-amber-600', value: 'text-amber-900' },
  };

  const cfg = colorMap[color] ?? colorMap.blue;

  return (
    <div className={`bg-gradient-to-br ${cfg.gradient} p-6 rounded-xl ${cfg.border}`}>
      <div className="flex items-center justify-between">
        <div>
          <p className={`text-sm font-medium ${cfg.label}`}>{label}</p>
...
function AssignmentsList({ assignments }: AssignmentsListProps) {
  ...
  return (
    <>
      {assignments.map((a, idx) => {
        ...
        return (
          <div key={key} className="bg-gray-50 border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
            <div className="font-semibold text-gray-800 mb-1">{a.subject}</div>
            <div className="flex flex-wrap gap-2">
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{a.branch}</span>
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Semester {a.semester}</span>
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">Section {a.section}</span>
            </div>
          </div>
        );
      })}
...
function ScheduledClassesTable({ classesList }: ScheduledClassesTableProps) {
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {classesList.map((s) => (
            <tr key={s.id} className="hover:bg-gray-50">
              <td className="px-6 py-4 whitespace-nowrap"><span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{s.day}</span></td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{s.start_time} - {s.end_time}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{s.subject}</td>
              <td className="px-6 py-4 whitespace-nowrap"><span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">{s.section}</span></td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{s.duration_hours} hrs</td>
            </tr>
          ))}
        </tbody>
      </table>
