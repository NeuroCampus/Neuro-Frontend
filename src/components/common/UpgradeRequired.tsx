...
const UpgradeRequired = ({ featureName, role, onBack }: UpgradeRequiredProps) => {
  ...
  return (
    <>
      ...
      >
        <div className="w-20 h-20 bg-[#a259ff]/10 rounded-full flex items-center justify-center mb-6">
          <Lock className="text-[#a259ff] w-10 h-10" />
        </div>
        ...
        <p className={`text-lg mb-8 max-w-md ${theme === 'dark' ? 'text-muted-foreground' : 'text-gray-500'}`}>
          {isAdmin ? (
            <>The <span className="font-bold text-[#a259ff] capitalize">{featureName.replace(/-/g, ' ')}</span> feature is exclusive to our Pro and Advance plans.</>
          ) : (
            <>The <span className="font-bold text-[#a259ff] capitalize">{featureName.replace(/-/g, ' ')}</span> feature is restricted. Please contact your Institution Admin for a plan upgrade.</>
          )}
        </p>
        <div className="flex flex-col sm:flex-row gap-4">
          {isAdmin && (
            <Button 
              ...
              className="bg-[#a259ff] hover:bg-[#a259ff]/90 text-white px-8 h-12 text-lg rounded-xl"
            >
              View Plans
            </Button>
          )}
